...
export const getKnockoutLeaderboard = async (req: AuthRequest, res: Response) => {
  try {
    const { limit = '10', round } = req.query;
    const parsedLimit = parseInt(limit as string, 10);
    const limitNum = Number.isFinite(parsedLimit) && parsedLimit > 0 ? parsedLimit : 10;
    const selectedRound = ['R16', 'R8', 'Semi'].includes(String(round || ''))
      ? String(round)
      : 'totalPoints';
    ...
    >`
      SELECT
        COALESCE(fur.R16, 0) AS R16,
        COALESCE(fur.R8, 0) AS R8,
        COALESCE(fur.Semi, 0) AS Semi,
        (COALESCE(fur.R16, 0) + COALESCE(fur.R8, 0) + COALESCE(fur.Semi, 0)) AS totalPoints,
      ...
      LEFT JOIN communities c1 ON c1.id = u.communityId1
      LEFT JOIN communities c2 ON c2.id = u.communityId2
      WHERE (COALESCE(fur.R16, 0) + COALESCE(fur.R8, 0) + COALESCE(fur.Semi, 0)) > 0
    `;
    ...
    const leaderboard = results
      ...
      .sort((a, b) => {
        const aScore = selectedRound === 'totalPoints' ? a.totalPoints : a[selectedRound as 'R16' | 'R8' | 'Semi'];
        const bScore = selectedRound === 'totalPoints' ? b.totalPoints : b[selectedRound as 'R16' | 'R8' | 'Semi'];
    ...
    const rankedLeaderboard = leaderboard.map((entry, index) => {
      const currentScore = selectedRound === 'totalPoints' ? entry.totalPoints : entry[selectedRound as 'R16' | 'R8' | 'Semi'];

      if (index === 0 || currentScore !== lastScore || entry.finalPoint !== lastFinalPoint) {
        currentRank = index + 1;
    ...
    const errorDetails = logger.error('getKnockoutLeaderboard', error, {
      method: req.method,
      path: req.path,
      userId: req.user?.userId,
    });
    res.status(errorDetails.statusCode || 500).json({ error: 'Failed to fetch knockout leaderboard' });
  }
};
